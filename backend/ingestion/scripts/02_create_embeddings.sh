...
printf 'Creating missing paid embedding checkpoints. Existing valid files are reused.\n'
if (( $# == 1 )); then
  uv run --locked python -m ingestion.embed_checkpoints --accession-number "$1"
else
