...
async def ingest_document_chunks(
    ...
    *,
    ...
    for document_index, source_row in enumerate(source_rows, start=1):
        ...
        await validate_existing_chunk_version(
            supabase_client,
            source_row,
            document_id,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate local chunks without creating embeddings or DB rows."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Required: validate local chunks without calling OpenAI or Supabase.",
    )
    parser.add_argument(
        "--accession-number",
...
def main() -> None:
    ...
    if not args.dry_run:
        raise ValueError(
            "Direct in-memory ingestion is disabled. Use "
            "ingestion/scripts/02_create_embeddings.sh followed by "
            "ingestion/scripts/03_upload_supabase.sh."
        )
